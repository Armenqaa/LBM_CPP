...
public:
    ...
    double k_rel_calculate(int) const;

    double tau_calculate(int size) const ;
    ...
    for (size_t i = 0; i < grid.size(); ++i) {
        for (size_t j = 0; j < grid[i].size(); ++j) {
            if (grid[i][j].interior) {
                for (const Vector2D dir : e()) {
                    if (i + dir.x < grid.size() && j + dir.y < grid[i].size()) {
                        count += grid[i + dir.x][j + dir.y].interior;
                    }
                }
                ...
                for (const auto direction : e()) {
                    ...
                    if (offset) {
                        ...
                        for (size_t q = 0; q < Q; ++q) {
                            if (e()[q].x == direction.x && e()[q].y == direction.y) {
